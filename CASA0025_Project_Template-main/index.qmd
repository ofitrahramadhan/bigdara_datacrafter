...
var clearButton = ui.Button({
  ...
  style: {margin: '10px', position: 'bottom-center'}
});


// the main panel to select mode, year, croptype
var panel = ui.Panel({
  style: {position: 'middle-left',width: '20%',backgroundColor: 'white',padding: '10px'},
  widgets: [
    ...
    ui.Label('Welcome to North Dakota Crop Yield Calculator. Please '+
    'select mode, year, and type of crop you want to estimate and '+
    'click or draw your area of interest on the map with the toolbox on top-left of the screen. '+
    'the result would be appear on the right side of webpage.', {
      fontSize: '12px',
    }),
        ...
        if (mode === 'County Level') {
          ...
          var getCalculation = function(countyName, cropYieldLayer) {
           ...
           //print(countStats);
            ...
            //____county label under construction
            var Result = ui.Label({
                value:'Result : ',
                style: {
                  fontWeight: 'bold',
            ...
            var countyPanel = ui.Panel({
              ...
              countyPanel.add(countyLabelup);
              countyPanel.add(countyLabel);
              resultsPanel.add(countyPanel);
          ...
          // get the mean yield data
            ...
            countStats.get('YIELDpredicted').evaluate(function(value){
              ...
              //_____under construction: crop area
              
              var cropareaPanel = ui.Panel({
                layout: ui.Panel.Layout.flow('horizontal')
              });
              
              var cropareaLabelup = ui.Label({
                value:'Crop Area : ',
                style: {fontWeight: 'bold'}
              });
              var cropareaLabel = ui.Label({
                  value: areaInSqKm.toFixed(2) + ' km² (' + areaInAcres.toFixed(2) + ' Acres)', 
                  style: {}
                });
              
              cropareaPanel.add(cropareaLabelup);
              cropareaPanel.add(cropareaLabel);
              resultsPanel.add(cropareaPanel);
              ...
              averYield.evaluate(function(result) {
                ...
                //___under construction
                
                var totalyieldPanel = ui.Panel({
                layout: ui.Panel.Layout.flow('horizontal')
              });
              
              var totalyieldLabelup = ui.Label({
                value:'Total Yield : ',
                style: {fontWeight: 'bold'}
              });
              var totalyieldLabel = ui.Label({
                  value: totalYield.toFixed(2) + ' BU', 
                  style: {}
                });
              
             totalyieldPanel.add(totalyieldLabelup);
              totalyieldPanel.add(totalyieldLabel);
                ...
                var totalyieldvaluePanel = ui.Panel({
                layout: ui.Panel.Layout.flow('horizontal')
              });
              
              var totalyieldvalueLabelup = ui.Label({
                value:'Total Yield Value : ',
                style: {fontWeight: 'bold'}
              });
              var totalyieldvalueLabel = ui.Label({
                  value: '$ ' + yieldPrice.toFixed(2), 
                  style: {}
                });
              
             totalyieldvaluePanel.add(totalyieldvalueLabelup);
              totalyieldvaluePanel.add(totalyieldvalueLabel);
                ...
                //  style: {fontSize: '13px', padding: '0px 50px'}
                //});
                resultsPanel.add(totalyieldPanel);
                resultsPanel.add(totalyieldvaluePanel);
          });
            });
          ...
          function initializeAreaLevelMode() {
            ...
            drawingTools.onDraw(function(geometry) {
             ...
             var meanStats = cropYieldLayer.reduceRegion({
              ...
              var meanYieldPanel = ui.Panel({
                layout: ui.Panel.Layout.flow('horizontal')
              });
              
              var meanYieldLabelup = ui.Label({
                value:'Average Yield : ',
                style: {fontWeight: 'bold'}
              });
              var meanYieldLabel = ui.Label({
                  value: values.meanYield.toFixed(2) + ' BU/Acre', 
                  style: {}
                });
              
              meanYieldPanel.add(meanYieldLabelup);
              meanYieldPanel.add(meanYieldLabel);
              resultsPanel.add(meanYieldPanel);
              ...
              areaPanel.add(areaLabelup);
              areaPanel.add(areaLabel);
              resultsPanel.add(areaPanel);
              ...
              var totalYield = areaInAcres * values.meanYield; //BU
              ...
              //count_totalYieldLabel.setValue(totalYield.toFixed(2)); 
               
              var yieldPrice = totalYield * cropPrice;
              
              var yieldPricePanel = ui.Panel({
                layout: ui.Panel.Layout.flow('horizontal')
              });
              
              var yieldPriceLabelup = ui.Label({
                value:'Total Yield Value : ',
                style: {fontWeight: 'bold'}
              });
              var yieldPriceLabel = ui.Label({
              ...
              yieldPricePanel.add(yieldPriceLabelup);
              yieldPricePanel.add(yieldPriceLabel);
              resultsPanel.add(yieldPricePanel);
